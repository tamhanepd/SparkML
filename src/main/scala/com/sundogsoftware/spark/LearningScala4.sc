...
val nums = Range(1,21,1).toList
println(nums)

// Looping over all numbers of the list
for (num <- nums) {
  var rem = num % 3
  if (rem == 0) {
    println(num)
...
// Another way with list comprehension like thing
for (num <- nums if num % 3 == 0) {
  println(num)
}

// Now with the filter function
val divs = nums.filter( (x: Int) => x % 3 == 0)
println(divs)